...
func (b *Bot) handleStart(message *tgbotapi.Message) {
	text := fmt.Sprintf("👋 Привет! Я помогаю не упустить выгодный кэшбэк.\n\n"+
		...
		"✍️ Добавить правило - напиши через запятую:\n"+
		"\"Тинькофф, Такси, 5%%, 3000, декабрь\"\n"+
		"или\n"+
		"\"Сбер, Перекресток доставка, 12%%, 30000, декабрь\"\n\n"+
...
func (b *Bot) handleNewRule(message *tgbotapi.Message) {
	...
	if len(missing) > 0 {
		text := "⚠️ Не хватает данных:\n" + strings.Join(missing, ", ") + "\n\n" +
			...
			"Пример: \"Тинькофф, Такси, 5%, 3000, декабрь\""
		b.sendMessage(message.Chat.ID, text)
		return
	}
