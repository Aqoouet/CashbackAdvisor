...
- Автоматическая валидация через API перед созданием правил
...
- `cashback_rules` — правила кэшбэков
...
8. API сервер создает правило в БД
...
3. Service ищет правило с точным совпадением категории
4. Если не найдено, ищет правило "Все покупки"
5. Возвращает правило с максимальным процентом кэшбэка
...
- `CreateCashback()` — создание правила
- `GetBestCashback()` — поиск лучшего кэшбэка
- `UpdateCashback()` — обновление правила
- `DeleteCashback()` — удаление правила
- `ListCashback()` — список правил с пагинацией
